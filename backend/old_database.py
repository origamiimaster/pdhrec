...
import requests
from moxfield import get_deck_data, check_deck_legal, get_new_decks
...
class Database:
    ...
    def update_deck(self, deck):
        ...
        self.last_time_update = max(parser.parse(
            str(deck['lastUpdatedAtUtc'])).timestamp(), self.last_time_update)
        ...
        if self.last_time_update < newest_time:
            ...
            while time_to_reach < last_deck_time:
                for deck in new_decks:
                    ...
                    last_deck_time = parser.parse(
                        deck['lastUpdatedAtUtc']).timestamp()
    ...
    def load_cards_to_commander(self, deck):
        ...
        if row[2:] in [[my_row[2], my_row[3]] if len(my_row) >= 4 else None for my_row in old_rows]:
            old_row = old_rows[[[my_row[2], my_row[3]]
                                for my_row in old_rows].index(row[2:])]
        if old_row is None:
            for card in deck['mainboard']:
                row.append(card)
                row.append(str(1))
            ...
            for card in deck['mainboard']:
                if card in old_row[2:]:
                    old_row[old_row.index(
                        card) + 1] = int(old_row[old_row.index(card) + 1]) + 1
                else:
                    old_row.append(card)
                    old_row.append(str(1))
            old_row[1] = int(old_row[1]) + 1
            update_row_generic(old_row, [[my_row[2], my_row[3]] for my_row in old_rows].index(
                row[2:4]) + 1, "Commanders")
        old_row = old_rows[0]
        for card in deck['mainboard']:
            if card in old_row[2:]:
                old_row[old_row.index(
